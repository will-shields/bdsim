...
#ifndef BDSIM_BDSDICOMPHANTOMZSLICEHEADER_HH
#define BDSIM_BDSDICOMPHANTOMZSLICEHEADER_HH
...
{
public:
    BDSDicomPhantomZSliceHeader(const G4String&);

    BDSDicomPhantomZSliceHeader( const BDSDicomPhantomZSliceHeader& ) = default;
    // build object copying an existing one (except Z dimensions)

    BDSDicomPhantomZSliceHeader( std::ifstream& fin );
    // build object reading data from a file

    ~BDSDicomPhantomZSliceHeader();
    ...
    void operator+=( const BDSDicomPhantomZSliceHeader& rhs );
    BDSDicomPhantomZSliceHeader operator+( const BDSDicomPhantomZSliceHeader& rhs );
...
inline void BDSDicomPhantomZSliceHeader::FlipData()
{
    std::reverse(fValues.begin(), fValues.end());
    std::reverse(fMateIDs.begin(), fMateIDs.end());
}
//=============================================================================
inline G4bool BDSDicomPhantomZSliceHeader::IsInteger(const G4String& str)
{
    return (str.find_first_not_of("0123456789") == std::string::npos) ? true : false;
}
//============================================================================
template <typename T>
inline T BDSDicomPhantomZSliceHeader::G4s2n(const G4String& str)
{
...
//============================================================================
template <typename T>
inline bool BDSDicomPhantomZSliceHeader::CheckConsistency(const T& val1, const T& val2,
                                                          G4String category) {
    if(val1 != val2) {
        G4Exception("BDSDicomPhantomSliceZHeader::CheckConsistency",
...
//============================================================================
template <typename T>
inline void BDSDicomPhantomZSliceHeader::Print(std::ostream& out, const std::vector<T>& val,
...
#endif //BDSIM_BDSDICOMPHANTOMZSLICEHEADER_HH
