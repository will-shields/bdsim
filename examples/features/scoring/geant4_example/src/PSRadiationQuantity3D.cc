...
PSRadiationQuantity3D::PSRadiationQuantity3D(G4String scorer_name, G4String filename,
        ...
        HCID3D(-1),EvtMap3D(nullptr), fDepthi(depi),fDepthj(depj),fDepthk(depk)
{
    ...
    conversion_factor = Data::Instance()->LoadTable(filename);
    ///
    G4cout << "-------------------------------------------------" << G4endl;

}

PSRadiationQuantity3D::~PSRadiationQuantity3D()
{
    conversion_factor.clear();
}

G4bool PSRadiationQuantity3D::ProcessHits(G4Step* aStep,G4TouchableHistory*)
{
...
void PSRadiationQuantity3D::Initialize(G4HCofThisEvent* HCE)
{
...
void PSRadiationQuantity3D::EndOfEvent(G4HCofThisEvent*)
{;}

void PSRadiationQuantity3D::clear()
{

    EvtMap3D->clear();
}

G4int PSRadiationQuantity3D::GetIndex(G4Step* aStep)
{
    const G4VTouchable* touchable = aStep->GetPreStepPoint()->GetTouchable();
...
inline G4double PSRadiationQuantity3D::GetInterpFactor(G4double E_x, vector<vector<G4double> > conversion_factor)
{
