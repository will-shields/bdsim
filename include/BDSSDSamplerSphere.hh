...
*/
#ifndef BDSSDSAMPLERSPHERE_H
#define BDSSDSAMPLERSPHERE_H
...
/**
 * @brief The sensitive detector class that provides sensitivity to BDSSamplerSphere instances.
 *
 * It creates BDSHitSamplerSphere instances for each particle impact on a sampler this SD is
 * attached to.
 * 
 * Written and edited by many authors over time.
 */

class BDSSDSamplerSphere: public BDSSensitiveDetector
{ 
public:
  /// Construct a sampler with name and type (plane/cylinder).
  explicit BDSSDSamplerSphere(const G4String& name);
  virtual ~BDSSDSamplerSphere();
