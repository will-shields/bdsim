...
//
const G4double BDSMagInt_Driver::max_stepping_increase = 5.0;
const G4double BDSMagInt_Driver::max_stepping_decrease = 0.1;

//  The (default) maximum number of steps is Base
//  divided by the order of Stepper
//
const G4int  BDSMagInt_Driver::fMaxStepBase = 250;  // Was 5000
...
//  Constructor
//
BDSMagInt_Driver::BDSMagInt_Driver( G4double                hminimum, 
...
//  Destructor
//
BDSMagInt_Driver::~BDSMagInt_Driver()
{ 
...
G4bool
BDSMagInt_Driver::AccurateAdvance(G4FieldTrack& y_current,
                                 G4double     hstep,
                                 G4double     eps,
                                 G4double hinitial )
{
  ...
  { 
    if(hstep==0.0)
    {
      ...
      G4Exception("BDSMagInt_Driver::AccurateAdvance()", 
...
// #endif
    ...
    else
    {
      ...
      if ( h == 0.0 )
      {
        ...
        if (dbg>2)
        {
          ...
          G4cout << "Warning: BDSMagIntegratorDriver::AccurateAdvance"
                 << G4endl
  ...
  if( dbg && no_warnings )
  {
    G4cerr << "BDSMagIntegratorDriver exit status: no-steps " << nstp <<G4endl;
    PrintStatus( yEnd, x1, y, x, hstep, nstp);
  }
#endif
...
void
BDSMagInt_Driver::WarnSmallStepSize( G4double hnext, G4double hstep, 
                                    ...
                                    G4int nstp)
{
  ...
  G4Exception("BDSMagInt_Driver::WarnSmallStepSize()", "GeomField1001",
              JustWarning, message);
  noWarningsIssued++;
}
...
void
BDSMagInt_Driver::WarnTooManyStep( G4double x1start, 
                                  G4double x2end, 
                                  G4double xCurrent)
{
    std::ostringstream message;
    ...
    G4Exception("BDSMagInt_Driver::WarnTooManyStep()", "GeomField1001",
                JustWarning, message);
}
...
void
BDSMagInt_Driver::WarnEndPointTooFar (G4double endPointDist, 
                                     G4double   h , 
                                     G4double  eps,
                                     G4int     dbg)
{
  ...
  { 
    ...
    message << "  Distance of endpoints = " << endPointDist
            ...
            << ", epsilon =  " << eps;
    G4Exception("BDSMagInt_Driver::WarnEndPointTooFar()", "GeomField1001",
                JustWarning, message);
  }
}
...
void
BDSMagInt_Driver::OneGoodStep(      G4double y[],        // InOut
                             const G4double dydx[],
                                   G4double& x,         // InOut
                                   G4double htry,
...

{
  ...
  for (iter=0; iter<max_trials ;iter++)
  {
    ...
    if( magvel_sq > 0.0 ) { 
       ...
       G4cerr << "** BDSMagIntegrationDriver: found case of zero momentum." 
    ...
    if(xnew == x)
    {
      G4cerr << "BDSMagIntegratorDriver::OneGoodStep:" << G4endl
...
//
G4bool  BDSMagInt_Driver::QuickAdvance(       
...
{
  G4Exception("BDSMagInt_Driver::QuickAdvance()", "GeomField0001",
                                  ...
                                  G4double&    dyerr )
{
  ...
  if(fVerboseLevel>2)
  {
    G4cout << "BDSMagIntDrv: Quick Advance" << G4endl;
...
// 
G4double 
BDSMagInt_Driver::ComputeNewStepSize( 
...
//
G4double 
BDSMagInt_Driver::ComputeNewStepSize_WithinLimits( 
                          G4double  errMaxNorm,    // max error  (normalised)
                          G4double  hstepCurrent)  // current step size
{
  G4double hnew;
...
void BDSMagInt_Driver::PrintStatus( const G4double*   StartArr,  
                                   G4double          xstart,
...
void BDSMagInt_Driver::PrintStatus(
                  ...
                  G4int                subStepNo)
{
    ...
    if( (subStepNo <= 1) || (verboseLevel > 3) )
    {
       ...
       G4cout << std::setw( 6)  << " " << std::setw( 25)
              << " BDSMagInt_Driver: Current Position  and  Direction" << " "
              << G4endl; 
...
void BDSMagInt_Driver::PrintStat_Aux(
...
void BDSMagInt_Driver::PrintStatisticsReport()
{
  G4int noPrecBig= 6;
  G4int oldPrec= G4cout.precision(noPrecBig);

  G4cout << "BDSMagInt_Driver Statistics of steps undertaken. " << G4endl;
  G4cout << "BDSMagInt_Driver: Number of Steps: "
         << " Total= " <<  fNoTotalSteps
         << " Bad= "   <<  fNoBadSteps 
         << " Small= " <<  fNoSmallSteps 
...
void BDSMagInt_Driver::SetSmallestFraction(G4double newFraction)
{
...
void BDSMagInt_Driver::GetDerivatives(const G4FieldTrack& y_curr, G4double* dydx) const
{
...
G4EquationOfMotion* BDSMagInt_Driver::GetEquationOfMotion()
{
    return pIntStepper->GetEquationOfMotion();
}

void BDSMagInt_Driver::SetEquationOfMotion(G4EquationOfMotion *equation)
{
    pIntStepper->SetEquationOfMotion(equation);
}

const G4MagIntegratorStepper* BDSMagInt_Driver::GetStepper() const
{
    return pIntStepper;
}

G4MagIntegratorStepper* BDSMagInt_Driver::GetStepper()
{
    return pIntStepper;
}
