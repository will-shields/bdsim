...
BDSPSCellFlux4D::BDSPSCellFlux4D(const G4String& name, const BDSHistBinMapper* mapperIn, G4int ni, G4int nj, G4int nk, G4int depi, G4int depj, G4int depk):
G4PSCellFlux3D(name,ni,nj,nk,depi,depj,depk), fDepthi(depi), fDepthj(depj), fDepthk(depk), mapper(mapperIn)
{
}

BDSPSCellFlux4D::BDSPSCellFlux4D(const G4String& name, const BDSHistBinMapper* mapperIn, const G4String& unit, G4int ni, G4int nj, G4int nk, G4int depi, G4int depj, G4int depk):
        G4PSCellFlux3D(name, unit, ni, nj, nk, depi, depj, depk), fDepthi(depi), fDepthj(depj), fDepthk(depk), mapper(mapperIn)
{
}
...
G4int BDSPSCellFlux4D::GetIndex(G4Step* aStep)
{
    ...
    G4int l = boost::apply_visitor([&energy](auto&& one){
        return (decltype(one)(one))->index(energy);
    }, mapper->GetEnergyAxis());
