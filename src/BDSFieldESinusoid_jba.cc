...
{
  G4double eZ = eField;//*std::cos(phase);//*std::cos(angularFrequency*t + phase);
  G4double eZ2 = eField*std::cos(phase);//*std::cos(angularFrequency*t + phase);
  G4ThreeVector field = G4ThreeVector(0, 0, eZ2);
  return field;
}
